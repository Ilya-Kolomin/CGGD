...
//		payload.color = {0.f, 0.f, (ray.direction.y + 1.f) * 0.5f};
	...
	std::uniform_real_distribution uniform_distribution(-1.f, 1.f);
	...
	raytracer->closest_hit_shader = [&](const ray& ray, payload& payload, const triangle<cg::vertex>& triangle, size_t depth) {
		...
		float3 random_direction {uniform_distribution(random_generator), uniform_distribution(random_generator), uniform_distribution(random_generator)};
